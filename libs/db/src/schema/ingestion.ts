...
export const rawFhirIngestions = pgTable(
    'raw_fhir_ingestions',
    {
        id: uuid('id').primaryKey().defaultRandom(),
        organizationId: uuid('organization_id')
    ...
    (table) => [
        uniqueIndex('unq_org_job_raw_row').on(
...
export const ingestionRowResults = pgTable(
    'ingestion_row_results',
    {
        id: uuid('id').primaryKey().defaultRandom(),
        organizationId: uuid('organization_id')
            .references(() => organizations.id, { onDelete: 'cascade' })
    ...
    (table) => [
        uniqueIndex('unq_org_job_row').on(
            table.organizationId,
            table.jobId,
            table.rowNumber,
        ),
